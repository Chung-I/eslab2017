...
ADXL345.prototype.measureAccel = function(callback) {
	...
	this.wire.readBytes(0x32, 6, function(err, res) {
		if (!err) {
			for (var axis = x_axis; axis <= z_axis; axis++) {
...
ADXL345.prototype.computeAccelBias = function(callback) {
	...
	function getSamples() {
		if (self.accelSampleCount < self.SAMPLECOUNT) {
			...
			self.runTimeAccelBias[x_axis] = -self.meterPerSecSec[x_axis];
			self.runTimeAccelBias[y_axis] = -self.meterPerSecSec[y_axis];
			self.runTimeAccelBias[z_axis] = -9.8065 - self.meterPerSecSec[z_axis];

			self.accelOneG = Math.abs(self.meterPerSecSec[z_axis] + self.runTimeAccelBias[z_axis]);
			callback();
		}
	}
